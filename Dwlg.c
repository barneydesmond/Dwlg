...
void maker(FILE* f , char txt[100][100],int t[100],int k,int lmin ,int lmax){
int j,i;
char ch1[100],ch[100];
...
int full(int t[100],int k,int n ,double x ,double fac){
int i=0,j=0,b=0,v=0;
...
if (v<n-k){return i;} else if(v>=n-k) {return (-1);}
...
if(i==n){ if(((100*x)*1.2/fac)<=100){  system("clear"); printf("NOW:[%lf%]\n",((100*x)*1.2/fac)); } else{ system("clear"); printf("NOW:[100%]\n"); }  }
    ...
    if(t[i]==-1){
     i=i-1;
        ...
        t[i-1]=-1;
        i=i-2;
       }else if(t[i]>=0) { maker(f,txt,t,i,lmin,lmax); i++; }
